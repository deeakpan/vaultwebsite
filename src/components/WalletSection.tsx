            ...
            {isConnected ? (
              <div className="space-y-4">
                <div className="p-4 bg-pepu-light-green/10 rounded-lg">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono text-sm text-primary">{formatAddress(address!)}</span>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div className="p-4 bg-secondary rounded-lg">
                  <h4 className="font-semibold text-primary mb-2">Supported Networks</h4>
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <div className="w-3 h-3 bg-pepu-light-green rounded-full"></div>
                      <span className="text-sm text-muted-foreground">PEPU Chain</span>
                    </div>
                  </div>
                </div>
                <div className="p-4 bg-pepu-light-green/10 rounded-lg">
                  <h4 className="font-semibold text-primary mb-2">Features Available</h4>
            ...
            <div className="bg-card rounded-2xl shadow-xl p-6 border border-border">
              ...
              <div className="space-y-4">
                <div className={`p-4 rounded-lg border ${
                  isConnected ? 'bg-pepu-light-green/10 border-pepu-light-green/30' : 'bg-secondary border-border'
                }`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                        isConnected ? 'bg-pepu-light-green' : 'bg-muted'
                      }`}>
                        <svg className="w-4 h-4 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                        </svg>
                      </div>
                      <span className="font-semibold text-primary">AI Analytics</span>
                    </div>
                    <span className={`text-sm ${isConnected && vaultBalanceData?.value && parseFloat(vaultBalanceData.value.toString()) / Math.pow(10, 18) >= 1000000 ? 'text-green-600' : 'text-muted-foreground'}`}>
                ...
                <div className={`p-4 rounded-lg border ${
                  isConnected ? 'bg-pepu-yellow-orange/10 border-pepu-yellow-orange/30' : 'bg-secondary border-border'
                }`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                        isConnected ? 'bg-pepu-yellow-orange' : 'bg-muted'
                      }`}>
                        <svg className="w-4 h-4 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                        </svg>
                      </div>
                      <span className="font-semibold text-primary">Rewards Tracking</span>
                    </div>
                ...
                <div className={`p-4 rounded-lg border ${
                  isConnected ? 'bg-pepu-dark-green/10 border-pepu-dark-green/30' : 'bg-secondary border-border'
                }`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                        isConnected ? 'bg-pepu-dark-green' : 'bg-muted'
                      }`}>
                        <svg className="w-4 h-4 text-pepu-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                      </div>
                      <span className="font-semibold text-primary">Governance Voting</span>
                    </div>
                    <span className={`text-sm ${isConnected ? 'text-green-600' : 'text-muted-foreground'}`}>
                      {isConnected ? 'Available' : 'Connect Required'}
                    </span>
